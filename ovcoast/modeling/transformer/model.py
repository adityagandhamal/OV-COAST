# Modified from CAT-Seg: https://github.com/cvlab-kaist/CAT-Seg/blob/main/cat_seg/modeling/transformer/model.py
...
class Aggregator(nn.Module):
    def __init__(self, 
        ...
        """
        Cost Aggregation Model from CAT-Seg
        Args:
    ...
    def Sinkhorn(self, K, u, v, n_batch, n_cls):
        r = torch.ones_like(u)
        c = torch.ones_like(v)
        thresh = 1e-2
        max_iter = 100
        for i in range(max_iter):
            r0 = r.view(n_batch, n_cls, 576)
            r = u.view(n_batch,n_cls,576) / torch.matmul(K.squeeze(-1), c.view(n_batch,n_cls,n_cls)).permute(0,2,1)
            c = v.view(n_batch, n_cls, n_cls) / torch.matmul(K.permute(0,2,3,1).squeeze(2), r.permute(0,2,1))
            err = (r - r0).abs().mean()
            if err.item() < thresh:
                break
        T = torch.matmul(r.permute(0,2,1), c) * K.squeeze(-1)
        return T
    ...
    def forward(self, img_feats, text_feats, appearance_guidance):
        ...
        """
        classes = None
        n_batch = img_feats.shape[0]
        n_cls = text_feats.shape[1]
        corr = self.correlation(img_feats, text_feats)
        sim = corr.view(n_batch, 1, n_cls, 576)
        sim = sim.permute(0, 3, 2, 1)
        wdist = 1.0 - sim
        xx = torch.zeros(n_batch * n_cls, 576, dtype=sim.dtype, device=sim.device).fill_(1. / 576)
        yy = torch.zeros(n_batch * n_cls, n_cls, dtype=sim.dtype, device=sim.device).fill_(1. / n_cls)

        with torch.no_grad():
            KK = torch.exp(-wdist / self.eps)
            T = self.Sinkhorn(KK,xx,yy,n_batch,n_cls)
        if torch.isnan(T).any():
            return None
 
        ot = T*sim.squeeze(-1)    #OT Distance
        ...
        corr = corr + ot.view(n_batch, 24, 24, n_cls).unsqueeze(1).permute(0, 1, 4, 2, 3)
